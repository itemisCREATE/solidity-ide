...
contract ByteExtractor {
    ...
    function getDigitFromByte32(bytes32 _b32, uint8 index) public pure returns(uint) {
    	uint numdigits = 64;
    	uint buint = uint(_b32);
    ...
    function getDigitFromUint(uint buint, uint8 index) public pure returns(uint) {
    	uint numdigits = 64;
