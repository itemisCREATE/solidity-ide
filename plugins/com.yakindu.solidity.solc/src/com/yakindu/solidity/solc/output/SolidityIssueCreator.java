...
public class SolidityIssueCreator {
	...
	public void createErrors(final List<CompileError> errors, final Set<File> filesToCompile, EObject currentObject,
	...
	protected void createSolcIssue(CompileError error, Set<File> filesToCompile, EObject currentObject,
		...
		if (errorFileMatchesCurrentFile(errorFile, currentObject)) {
			if (errorFile != null) {
				...
				EObject element = getEObject(offset, currentObject);
				if (sourceLocation.getStart() == -1) {
				...
				if (severity.equals(ERROR)) {
					...
					acceptor.acceptWarning(message, element, null, offset,
							createErrorCodeFromMessage(severity, message));
				}
			}
		}
	}
	...
	protected void createInfoIssue(CompiledContract contract, File file, EObject currentObject,
		...
		if (errorFileMatchesCurrentFile(file, currentObject)) {
			EObject element = getEObject(0, currentObject);
			acceptor.acceptInfo(message, element.eContainer(), 0, 0, createErrorCodeFromMessage(INFO, message));
		}
	}
	
	protected IFile getCurrentFile(EObject currentObject) {
		IWorkspace workspace = ResourcesPlugin.getWorkspace();
		IWorkspaceRoot root = workspace.getRoot();
		IFile currentFile = root.getFile(new
				Path(currentObject.eResource().getURI().toPlatformString(true)));
		return currentFile;
	}
	...
	protected File findFileForName(Set<File> filesToCompile, String fileName) {
		File errorFile = filesToCompile.stream()
				.filter(file -> file.getAbsolutePath().replaceAll(SPLITTER, "/").equals(fileName))
	...
	protected EObject getEObject(int offset, EObject currentObject) {
