...
class SolidityIdeCodeActionService implements ICodeActionService2 {
	...
	override List<Either<Command, CodeAction>> getCodeActions(Options options) {
			...
			if (d.code == IssueCodes.WARNING_SOLIDITY_VERSION_NOT_THE_DEFAULT) {
				actions += Either.forRight(new CodeAction => [
					...
					edit = new WorkspaceEdit() => [
						addTextEdit(options.resource.URI, new TextEdit => [
							range = d.range
							newText = SolidityVersion.toString(solcVersion)
						])
					]
				])
			}
